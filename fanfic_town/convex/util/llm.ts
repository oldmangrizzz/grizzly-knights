...
export function getLLMConfig(): LLMConfig {
  ...
  return {
    ...
    chatModel: process.env.OLLAMA_MODEL ?? 'kimi-k2.6:cloud',
...
export async function chatCompletion(
  ...
  // Per-call provider routing by model-name prefix.
  // - copilot/<model>  -> local copilot-api proxy on COPILOT_API_HOST (no auth header)
  // Anything else uses the global config (Ollama / OpenAI / etc).
  let endpointUrl = config.url;
  let extraHeaders: Record<string, string> = AuthHeaders();
  let providerOverride: 'copilot' | null = null;
  if (typeof body.model === 'string' && body.model.startsWith('copilot/')) {
    body.model = body.model.slice('copilot/'.length);
    endpointUrl = process.env.COPILOT_API_HOST ?? 'http://127.0.0.1:4141';
    extraHeaders = {};
    providerOverride = 'copilot';
    // Copilot's GPT-5 family rejects `stop` and requires `max_completion_tokens`.
    delete (body as any).stop;
    const anyBody = body as any;
    if (anyBody.max_tokens !== undefined && anyBody.max_completion_tokens === undefined) {
  ...
  const {
    ...
    const result = await fetch(endpointUrl + '/v1/chat/completions', {
      method: 'POST',
      headers: {
    ...
    if (!result.ok) {
      ...
      console.error({ error });
      if (result.status === 404 && config.provider === 'ollama' && !providerOverride) {
